...
int main(int argc, char ** argv){
    ...
    rc.stride = conf.alloc_amount;
    for(rc.lane_count = 2; rc.lane_count <= conf.max_lane_count; rc.lane_count++){
        rc.real_data_amount = rc.stride * rc.lane_count;
        run_benchmark();
    }

    allocator.deallocate(data, conf.alloc_amount * conf.max_lane_count);
}